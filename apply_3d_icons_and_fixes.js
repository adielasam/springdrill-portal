...
// Helper to generate 3D volumetric icons
function get3DIcon(type, colorKey, hexStart, hexEnd) {
    ...
    <svg width="48" height="48" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg">
        <defs>
            ...
            <filter id="vol-inner-\${colorKey}">
                <!-- Inner top highlight to create 3D bevel -->
                <feDropShadow dx="0" dy="-2" stdDeviation="2" flood-color="#ffffff" flood-opacity="0.4" />
            </filter>
        </defs>
        <!-- The rounded 3D squircle -->
...
// Inside JS block: we update statClasses right after we know the chart labels
...
// Remove the hardcoded broken statClasses calculation in Promise.all block
...
// 4. Update the Chart bars to use rich linear gradients that perfectly match the palette
...
const chartJSColorsNew = `
                ...
                for(let i = 0; i < labels.length; i++) {
                    finalBgColors.push(bgColors[i % 6]);
                }
`;
...
console.log('Successfully applied 3D icons, fixed classes stat, and applied chart gradients.');
